...
# HDFS home for the self-healing daily roll-up tables (written by the two
# *_daily_rollup jobs, read by rucio_no_access.py)
HDFS_BASE=/user/dmtops
...
py_input_args=(--creds /etc/secrets/amq.json --rollup $HDFS_BASE/access_daily --resid $HDFS_BASE/residency_daily)
# log all to stdout
spark-submit "${spark_submit_args[@]}" "$script_dir/rucio_no_access.py" "${py_input_args[@]}" 2>&1
